...
export function useTooltip({
  ...
  // Non-null by construction: solo.current is assigned just above whenever
  // there is no surrounding group.
  const store = (group ?? solo.current)!
        ...
        <AnimatePresence>
          {open && (
            <motion.span
              ...
              className={`relative w-max max-w-[220px] shrink-0 overflow-hidden rounded-[8px] px-2 py-1 text-[11.5px] leading-snug font-medium text-stone-700 dark:text-stone-100 ${contentClassName}`}
            >
              <motion.span
                aria-hidden
                layout={!reduced}
