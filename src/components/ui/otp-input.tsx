          ...
          return (
            <div key={i} className={`relative h-12 w-10 ${gap ? "ml-3" : ""}`}>
              <input
                ...
                className={`h-12 w-10 rounded-[10px] border-2 text-center text-[15px] text-transparent caret-transparent transition-[background-color,border-color,box-shadow] duration-150 outline-none selection:bg-transparent focus-visible:outline-none disabled:opacity-50 ${
                  error
                ...
                <AnimatePresence initial={false} mode="popLayout">
                  {char ? (
                    <motion.span
                      ...
                      transition={enter}
                      className="col-start-1 row-start-1 font-mono text-[15px] text-stone-700 tabular-nums dark:text-stone-200"
                    >
                      {char}
                    </motion.span>
                  ) : null}
