...
function DeckCard({
  ...
  const badge = (side: -1 | 1, text: string, place: string) => {
    ...
    return (
      <motion.span
        ...
        className={`pointer-events-none absolute top-3 rounded-[6px] border bg-white px-2 py-1 text-[10.5px] font-semibold tracking-[0.08em] whitespace-nowrap uppercase transition-colors duration-150 dark:bg-[#1D1D1A] ${
  ...
  return (
    <motion.div
      ...
      className={`absolute inset-x-5 top-0 overflow-hidden rounded-[14px] border border-stone-200 bg-white select-none dark:border-white/[0.16] dark:bg-[#1D1D1A] ${
        active
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div className="mt-3 grid h-8 grid-cols-[1fr_auto_1fr] items-center gap-3">
        ...
        <span className="flex items-center gap-2 font-mono text-[10.5px] text-stone-500 tabular-nums dark:text-stone-400">
          ...
          <button
            ...
            onClick={deck.undo}
            inert={!deck.canUndo}
            className={`inline-flex items-center gap-1 rounded-[5px] px-1 py-0.5 text-stone-700 transition-[background-color,box-shadow,opacity] duration-150 outline-none hover:bg-stone-100 focus-visible:bg-[#4568FF]/[0.06] focus-visible:shadow-[inset_0_0_0_1px_#4568FF] dark:text-stone-200 dark:hover:bg-white/10 dark:focus-visible:bg-[#93B0FF]/[0.1] dark:focus-visible:shadow-[inset_0_0_0_1px_#93B0FF] ${spent(
              !deck.canUndo
            )}`}
          >
            {ICON_UNDO}
